...
global odo, vx, vy, lidar_info, lidar_ranges, lidar_angle_min,lidar_angle_increment, brake_bool_result, Ackermann_drive_result, TTC_threshold, min_TTC
...
class Safety(object):
    ...
    def __init__(self):
        ...
        sub2 = rospy.Subscriber('/odom', Odometry, self.odom_callback, queue_size = 10)
    ...
    def scan_callback(self, scan_msg):
        ...
        for x in range(len(self.lidar_ranges)):
            if (not (math.isinf(self.lidar_ranges[x])) and not (math.isnan(self.lidar_ranges[x]))):
